...
#Código relacionado à execução do Menu dos Jogos
class sli(object):
            ...
            if event.type == QUIT:
                ...
                if event.button==1:
                    if Xoffset < event.pos[0] < Xoffset+boxwidth and Yoffset < event.pos[1] < totalheight*len(Items):
                        clicked_item = int((event.pos[1] - 20)/totalheight)
                ...
                if Xoffset < event.pos[0] < Xoffset+boxwidth and Yoffset < event.pos[1] < totalheight*len(Items):
                    focus = int((event.pos[1] - Yoffset)/totalheight)
...
#Método relacionado a terminar o jogo
def quitgame ():
    pygame.quit()
    sys.exit()